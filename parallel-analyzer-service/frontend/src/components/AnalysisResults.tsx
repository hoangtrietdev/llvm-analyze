        ...
        >
          ...
          <div className="divide-y divide-gray-700">
            {group.results.map((result, index) => {
              ...
              console.log('LLVM Candidate Type:', result.llvm_analysis?.candidate_type);
              console.log('LLVM Reason:', result.llvm_analysis?.reason);
              console.log('Enhanced Analysis:', result.enhanced_analysis);
              console.log('Enhanced Confidence:', result.enhanced_analysis?.confidence);
              console.log('Confidence Breakdown:', result.enhanced_analysis?.confidence_breakdown);
              console.log('OpenMP Validation:', result.enhanced_analysis?.openmp_validation);
              console.log('OpenMP Confidence Boost:', result.enhanced_analysis?.openmp_validation?.confidence_boost);
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-purple-400 italic">
                            This candidate should likely be ignored or manually
                            reviewed.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-900/60 rounded-lg p-4 border border-gray-700">
                        ...
                        <div className="space-y-2 text-xs">
                          {(() => {
                            ...
                            const codeContext = breakdown?.code_context ?? 0;
                            const metadata = breakdown?.metadata ?? 0;
                            const openmpValidation = breakdown?.openmp_validation ?? 0;
                            const finalConfidence = result.enhanced_analysis?.confidence ?? 0;
                              ...
                              <>
                                ...
                                <div className="flex items-center justify-between bg-gray-800/50 rounded p-2">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className={codeContext >= 0 ? "text-green-300" : "text-red-300"}>
                                      {codeContext >= 0 ? '+' : ''}{(codeContext * 100).toFixed(0)}%
                                    </span>
                                  </div>
                                  <span className={`font-bold ${codeContext >= 0 ? "text-green-400" : "text-red-400"}`}>
                                    {codeContext >= 0 ? '+' : ''}{(codeContext * 100).toFixed(0)}%
                                  </span>
                                </div>
                                ...
                                <div className="flex items-center justify-between bg-gray-800/50 rounded p-2">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className={metadata >= 0 ? "text-green-300" : "text-red-300"}>
                                      {metadata >= 0 ? '+' : ''}{(metadata * 100).toFixed(0)}%
                                    </span>
                                  </div>
                                  <span className={`font-bold ${metadata >= 0 ? "text-green-400" : "text-red-400"}`}>
                                    {metadata >= 0 ? '+' : ''}{(metadata * 100).toFixed(0)}%
                                  </span>
                                </div>
                                ...
                                <div className="flex items-center justify-between bg-gray-800/50 rounded p-2">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className={openmpValidation >= 0 ? "text-green-300" : "text-gray-400"}>
                                      {openmpValidation > 0 ? '+' : ''}{(openmpValidation * 100).toFixed(0)}%
                                    </span>
                                  </div>
                                  <span className={`font-bold ${openmpValidation > 0 ? "text-green-400" : "text-gray-500"}`}>
                                    {openmpValidation > 0 ? '+' : ''}{(openmpValidation * 100).toFixed(0)}%
                                  </span>
                                </div>
                                ...
                                <div className="bg-gray-800/30 rounded p-2 mb-2">
                                  <div className="text-xs text-gray-400 font-mono text-center">
                                    {(basePattern * 100).toFixed(0)}% + {codeContext >= 0 ? '+' : ''}{(codeContext * 100).toFixed(0)}% + {metadata >= 0 ? '+' : ''}{(metadata * 100).toFixed(0)}% + {openmpValidation >= 0 ? '+' : ''}{(openmpValidation * 100).toFixed(0)}% = {(finalConfidence * 100).toFixed(0)}%
                                  </div>
                          ...
                          {result.code_block && (
                            <div className="border-t border-gray-600 pt-2 mt-2">
                              ...
                              {result.code_block.analysis_notes &&
                                result.code_block.analysis_notes.length > 0 && (
                  ...
                  {result.ai_analysis.classification === "not_parallel" && (
                    <div className="bg-red-800 rounded p-2 mb-2">
                      <p className="text-xs text-red-200 font-medium">
                        ❌ Not parallelizable - Dependencies or sequential logic
                        detected
                      </p>
                    </div>
                  )}
                ...
                <div className="mt-3 pt-2 border-t border-gray-600 flex items-center justify-between text-xs text-gray-400">
                  <div className="flex items-center space-x-4">
                    ...
                    {result.hybrid_confidence && (
                      <span
                        className={getConfidenceColor(result.hybrid_confidence)}
                      >
                        Hybrid Score:{" "}
